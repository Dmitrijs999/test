resource "aws_cloudfront_distribution" "internal" {
  ...
  origin {
    ...
    origin_id   = "contracts"
    origin_path = "/addresses/mantle"
  }

  origin {
    domain_name = var.contracts_s3_bucket_domain_name
    origin_id   = "gas-costs"
    origin_path = "/gas-costs/mantle"
  }
  ...
  origin {
    domain_name         = var.poffchainer_domain_name
    origin_id           = "poffchainer"
    connection_attempts = 1
  ...
  ordered_cache_behavior {
    path_pattern     = "/poffchainer/*"
    allowed_methods  = ["HEAD", "DELETE", "POST", "GET", "OPTIONS", "PUT", "PATCH"]
    cached_methods   = ["GET", "HEAD"]
    target_origin_id = "poffchainer"
