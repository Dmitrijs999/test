...
import { EventType, ExtMarketMeta, IEventHistoryItem } from 'types';
...
const TransactionHistory: React.FC<{
  ...
  mb: string;
}> = ({ accountAddress, marketMeta, mb = '0' }) => {
  const [_list, setList] = useState<IEventHistoryItem[]>([]);
  ...
  const {
  ...
  } = useGetUserTransactionHistoryQuery(
    {
      ...
      symbol: marketMeta?.symbol as string,
      skip,
      limit: LIMIT,
      orderBy,
      orderDirection,
  ...
  const dateWidth = useMediaValue('40%', '25%', '25%');
  const txIdWidth = useMediaValue('10%', '25%', '25%');
    ...
    ({ type }: { type: EventType }) => {
      return isFetching ? (
        ...
        <Typography
          variant='copyM'
          style={{ color: '#222A34' }}
          text={t(mapEventType[type as EventType])}
        />
      );
    },
  ...
  const AmountCell = useCallback(
    ({ amount, amountUsd }: { amount?: string; amountUsd?: string }) => {
      return isFetching ? (
        <SkeletonRect
        ...
        <NumericInfo
          ...
          subValue={`${unit(amount)} ${marketMeta?.name}`}
          isLoading={false}
          align='left'
        />
      );
